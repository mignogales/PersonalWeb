...
export async function handleOffice(request, env) {
  const url = new URL(request.url);
  const routes = new Set(["/login", "/logout", "/schedule", "/schedule/me"]);
  ...
  const base = env.OFFICE_SCHEDULER_API_BASE || "https://api.miguelnogales.com";
  ...
  try {
    ...
    const response = await fetch(`${base}/office${path}`, {
    ...
    return Response.json({ error: "The shared calendar is temporarily unavailable. Your unsaved selections are still here." }, {
      status: 503, headers: { "Cache-Control": "no-store", "X-Office-Upstream-Status": String(upstreamStatus || "network-error"), "X-Office-Error-Category": category },
    });
  }
}
